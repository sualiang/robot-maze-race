...
Page({
  ...
  onTabChange: function (e) {
    ...
    else if (tab === 'expand') this.fetchExpandRecords();
  },
  ...
  fetchExpandRecords: function () {
    var that = this;
    that.setData({ expandLoading: true, expandEmpty: false });
    request.get('/player/me/expand-records').then(function (records) {
      var list = records || [];
      list.forEach(function (item) {
        item.usedAtText = '';
        if (item.usedAt) {
          var d = new Date(item.usedAt);
          item.usedAtText = d.getFullYear() + '-' + (d.getMonth() + 1).toString().padStart(2, '0') + '-' + d.getDate().toString().padStart(2, '0');
        }
      });
      that.setData({ expandRecords: list, expandLoading: false, expandEmpty: list.length === 0 });
    }).catch(function (err) {
      console.error('获取膨胀记录失败', err);
      that.setData({ expandLoading: false });
    });
  },
