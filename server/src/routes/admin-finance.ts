...
router.get('/payment-records', authMiddleware, checkPermission('admin:finance'), async (req: Request, res: Response) => {
  try {
    ...
    const selectCols = `p.id, p.order_id, o.operator_id, o.user_id, p.transaction_id,
                         p.amount_cents, p.status, p.pay_time, p.created_at`;
    const fromJoin = `FROM payments p JOIN orders o ON p.order_id = o.id`;
    const baseWhere = `1=1 ${whereClause}`;
    const orderBy = `ORDER BY p.pay_time DESC`;
    ...
    if (opId) {
      // 普通运营商：单库
      const opWhere = `o.operator_id = ? ${whereClause}`;
      const countParams = [opId, ...params];
      const countRow = await queryOpOne<{ count: string }>(req,
        `SELECT COUNT(*) as count FROM (SELECT ${selectCols} ${fromJoin} WHERE ${opWhere}) t`,
        countParams
      );
      total = parseInt(countRow?.count || '0', 10);
      const pageSql = `SELECT ${selectCols} ${fromJoin} WHERE ${opWhere} ${orderBy} LIMIT ? OFFSET ?`;
      ...
      for (const reg of opRegs) {
        try {
          ...
          const dbWhere = `o.operator_id = ? ${whereClause}`;
          const [dbRows] = await pool.execute(`SELECT ${selectCols} ${fromJoin} WHERE ${dbWhere} ${orderBy}`, dbParams);
